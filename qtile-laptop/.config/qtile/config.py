...
def count_active_screens():
    ...
    active_screens = [l for l in lines if ' connected ' in l and re.search('\d+x\d+', l)]
    return len(active_screens)


def init_screens():
    extra_monitors = count_active_screens() - 1
    screens = [
        Screen(
...
def restart_on_randr(_):
    asyncio.create_task(reload_config_once())
